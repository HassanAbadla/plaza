...
<div class="page w-full p-8">
  <div class="flex align-center justify-between bg-slate-200 p-8 rounded-lg">
    <a href="#new_property" @click="newProperty" class="modal_button"> موظف جديد </a>
    <input type="text" name="search">
  </div>
  <div class="py-8">
    <h3 class="text-lg"> الموظفون </h3>
    <div class="data" role="region" tabindex="0">
      <table class="border-r table-auto">
        ...
        <tbody>
          <tr v-for="employee in allEmployees" :key="employee.id" class="text-xs">
            <td>{{ employee.id }}</td>
            <td>{{ employee.name }}</td>
...
export default {
  ...
  computed: {
    allEmployees() {
      return this.$store.state.allEmployees
    }
  }
