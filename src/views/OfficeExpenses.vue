<template>
  <div class="page w-full p-8">
    ...
    <div class="py-8">
      <h3 class="text-lg"> مصاريف المكتب </h3>
...
const properitiesList = ref([])
   ...
   const addEmployee = () => {
      ...
      store.state.officeExpenses.push(data)
      store.dispatch('addOfficeExpenses', data).then(() => {
        let body = document.body
        body.classList.remove('overflow')
...
const expenses = ref(store.state.officeExpenses)
// const expenses = computed(() => { return store.state.officeExpenses })
