...
<div class="page w-full p-8">
  <div class="flex align-center justify-between bg-slate-200 p-8 rounded-lg">
    <a href="#new_property" @click="newProperty" class="modal_button"> عميل جديد </a>
    <input type="text" name="search">
  </div>
  <div class="py-8">
    <h3 class="text-lg"> العملاء </h3>
      ...
      <table class="border-r table-auto">
        <thead class="text-sm">
          <tr>
            <th class="w-12">#</th>
            <th>اسم العميل</th>
            <th class="w-32">رقم الموبايل</th>
            <th class="w-32">الجنسية</th>
            <th class="">ملاحظات</th>
            <th class="w-32 text-center">...</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="client in allClients" :key="client.id" class="text-xs">
            <td>{{ client.id }}</td>
            <td>{{ client.name }}</td>
            <td>{{ client.mobile }}</td>
        ...
        <div class="form">
          ...
          <div class="form-group">
            ...
            <div class="input-group">
              ...
              <input type="text" name="nationality" v-model="nationality">
            </div>
          </div>
        </div>
        <div class="form">
...
export default {
  ...
  methods: {
    ...
    addBroker() {
      ...
      this.$store.dispatch('addClient', data).then(() => {
        let body = document.body
        body.classList.remove('overflow')
